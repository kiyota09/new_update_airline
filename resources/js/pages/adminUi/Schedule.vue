...
const allRoute = computed(() => page.props.RouteList);
const selectedRouteId = ref('');

// Selected route for Edit Flight
const selectedRouteIdEdit = ref('');

// Watch Add Flight route selection
watch(selectedRouteId, (id) => {
    const route = allRoute.value.find((r) => r.id === id);
    if (route) {
...
// Watch Edit Flight route selection
watch(selectedRouteIdEdit, (id) => {
    const route = allRoute.value.find((r) => r.id === id);
    if (route && selectedFlight.value) {
        selectedFlight.value.origin = route.origin_route;
        selectedFlight.value.destination = route.destination_route;
    }
});
...
const openEditModal = (flight: Flight) => {
    ...
    // Find route ID for the current flight
    const route = allRoute.value.find(
        (r) =>
            r.origin_route === flight.origin &&
            r.destination_route === flight.destination,
    );
    selectedRouteIdEdit.value = route ? route.id : '';
...
const saveChanges = async () => {
    ...
    try {
        await axios.put(
            `/admin/schedule/${selectedFlight.value.id}`,
...
const sendNotification = () => {
    if (selectedFlight.value) {
        alert(
            `Passengers on flight ${selectedFlight.value.flightNo} have been notified.`,
        );
    }
    closeModals();
};
        ...
        <div class="ml-64 flex-1 p-6">
            ...
            <div class="mb-6 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <button
                        @click="prevMonth"
                        class="rounded-lg bg-gray-200 px-3 py-1 text-gray-700 hover:bg-gray-300"
                    >
                        ‹
                    </button>
                ...
                >
                    ...
                    >
                        ...
                        <div class="mt-2 flex justify-end gap-1">
                            <button
                                @click.stop="openEditModal(flight)"
                                class="rounded bg-yellow-500 px-2 py-1 text-[10px] text-white hover:bg-yellow-600"
                            >
                                Edit
                            </button>
                            <button
                                @click.stop="openNotifyModal(flight)"
                                class="rounded bg-blue-600 px-2 py-1 text-[10px] text-white hover:bg-blue-700"
                            >
                                Notify
                            </button>
                            <button
                                @click.stop="openDeleteModal(flight)"
            ...
            >
                ...
                >
                    <h3 class="mb-4 text-xl font-bold text-green-600">
                        Add New Flight
                    </h3>
                    <div class="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                class="w-100 rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 shadow-sm transition focus:border-green-500 focus:ring focus:ring-blue-200 focus:outline-none"
                            />
                        </div>
            ...
            <!-- Edit Flight Modal -->
            <div
                v-if="showEditModal && selectedFlight"
                class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
                @click.self="closeModals"
            >
                <div
                    class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"
                >
                    <h3 class="mb-4 text-xl font-bold text-yellow-600">
                        Edit Flight
                    </h3>
                    <div class="space-y-4">
                        <!-- Flight No -->
                        ...
                        <div class="flex flex-col">
                            ...
                            <select
                                v-model="selectedRouteIdEdit"
                                class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 shadow-sm transition focus:border-yellow-500 focus:ring focus:ring-yellow-200 focus:outline-none"
                            >
                                <option disabled value="">
                                    Select a route
                                </option>
            ...
            <!-- Delete Flight Modal -->
            <div
                v-if="showDeleteModal && selectedFlight"
                class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
                @click.self="closeModals"
            >
                <div
                ...
                >
                    ...
                    <p>
                        ...
                        <strong>{{ selectedFlight.flightNo }}</strong
                        >?
                    </p>
            ...
            <!-- Notify Modal -->
            <div
                v-if="showNotifyModal && selectedFlight"
                class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
                @click.self="closeModals"
            >
                <div
                ...
                >
                    ...
                    <p>
                        Send notification to passengers on flight
                        <strong>{{ selectedFlight.flightNo }}</strong
                        >?
                    </p>
                    <div class="mt-6 flex justify-center gap-4">
